...
public class PenMultisImp extends BaseImp {
    ...
    private Map<Integer,BaseAction> mMultiAction;
    private volatile int pointid;
    private ResourceDataManager mResourceManager;
    private float mLastx,mLasty;
    public PenMultisImp() {
        //用来记录操作步骤的类，用来还原
        mResourceManager = ResourceDataManager.getInstance();
    }
        ...
        switch (event.getActionMasked()) {
                    ...
                    if (pointerId != -1) {
                        ...
                        if (dx >= 2 || dy >= 2) {
                            ...
                            if (action != null) {
                                ...
                                mResourceManager.lineMove(x, y, pointerId);
                            }
                        }
                        mLastx = x;
                        mLasty = y;
            ...
            case MotionEvent.ACTION_POINTER_UP:
                ...
                if (baseAction != null) {
                    ...
                    mResourceManager.lineUp(x,y,pointid);
    ...
    private BaseAction setAction(MotionEvent event) {
        ...
        mResourceManager.lineStart(x,y,pointerId, DrawActionType.PEN.ordinal(),getPenType().ordinal(),getPenColor(),
                getPenSize());
        //协同白板要发送的数据
        TransBean bean = getTransBean(DrawActionType.PEN);
        mLastx = bean.x = x;
